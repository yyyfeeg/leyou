...
class File
{
    ...
    public function write($file, $content, $append = false) 
    {
        ...
        // 如果无法写入文件，则记录错误
        if (!$fp = @fopen($file, $type)) return false;
     ...
     * @return array
     */
    public function readAllArray($file) 
    {
		$file = str_replace('\\', '/', $file);
		if (!$this->isExists($file)) {
            return "";
    ...
    private function cpDir($oldDir, $aimDir, $overWrite) 
    {
        ...
        if (!$this->isExists($aimDir)) $this->mkDir($aimDir);
        $dirHandle = opendir($oldDir);
        while(false !== ($file = readdir($dirHandle))) 
        {
    ...
    {
        if (!$this->isExists($fileUrl)) return false;
        if ($this->isExists($aimUrl) && $overWrite == false) {
            return false;
        } elseif ($this->isExists($aimUrl) && $overWrite == true) {
            $this->rmFile($aimUrl);
     ...
     */
	public function findDir($dir, &$Filearr, $child = true)
	 {
		$dir = str_replace('\\', '/', $dir);
     ...
     * @return  array      $Filearr
     */
	public function getDir($dir, $child = true)
	{
		$dir = str_replace('\\', '/', $dir);
	 	$Filearr = array();
