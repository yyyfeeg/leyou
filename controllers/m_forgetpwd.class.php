...
class M_forgetpwd extends Controller
{
	/**
	 * 构造函数，初始化
	 */
	...
	{
		parent::__construct();
		$this->assign('menu_title','找回密码');// 栏目标题
	...
	public function index()
	{
		...
		$this->assign('menu_title',"找回密码");
		$this->display('mobile/forgetpassword.html');
	}
	...
	public function phone_getpwd()
	{
		...
		if ($flag == 'phone_getpwd' && $step == 'step2' && !empty($name) && !empty($type)) {
			// 查询用户信息
			$user_info = $GLOBALS['redis']->lRange("{$name}_info",0,-1);
			$phone = $user_info[2];
			switch ($type) {
	...
	public function set_newpwd()
	{
		...
		if ($flag == 'set_newpwd' && $step == 'step3' && !empty($name) && !empty($sign)) {
			$pwd_len = strlen($pwd);
			if (empty($pwd)) {
	...
	public function setpwd_success()
	{
		...
		$this->assign('name',$_SESSION['findpwd_name']);
		$this->assign('gotoUrl',WEBPATH_DIR_INC.'/html/mobile/index.html');
		$this->display('mobile/forgetpassword.html');
	}
}
?>