<?php
#=============================================================================
#     FileName: update_all_remain.php
...
if ($forceupdate == 2) {
    // 删除当天表中的数据
    $sql_del = "delete from ".get_table('all_remain')." where car_logdate > $date1 and car_logdate < $date2";
    $DB->Query($sql_del);
...
if($pt_re){
    ...
    if($pt_nums>0){
        while($row = $DB->FetchArray($pt_re)){
            ...
            //判断是否存在记录
            if(exist_check($DB,'count_all_remain', $wherearr,2)){
               //更新数据
               $info = update_record($DB,'count_all_remain',$pt_arr,$wherearr,"",2);
               if(!$info){
                    $msg .="<br>"."平台注册留存数据更新出错".__LINE__;
                    break;
                }
                $unum++;
            }else{
                $pt_info = add_record($DB,"count_all_remain",$pt_arr,'',2);
    		    if($pt_info['rows']<1){
                        $msg .= "<br>"."平台注册留存数据更新失败，失败原因 ：插入数据库出错".__LINE__;
                        break;
                    }
                $inum++;
            }
